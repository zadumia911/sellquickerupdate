					...
					<div class="row">
						<div class="col-sm-6 col-6">
							...
							<button href="#" type="button" data-toggle="modal" data-target="#exampleModalLong">
							 <img src="{{asset('public/frontEnd/')}}/images/LOCATION.png" alt="" class="location-icon"> @if($findthana !=NULL) {{$finddivision->name}} <i class="fa fa-angle-right"></i>  {{$finddistrict->dist_name}} <i class="fa fa-angle-right"></i> {{$findthana->thana_name}} @else অবস্থান নির্বাচন করুন @endif
							</button>
							</div>
							<!-- Modal -->
								      ...
								      <div class="modal-body">
											 ...
											 @foreach($categories as $key=>$category)
											  <div class="tab-pane fade " id="v-pills-category{{$category->id}}" role="tabpanel" aria-labelledby="v-pills-category-tab{{$category->id}}">
											  	...
											  	<a href="" class="all-area">{{$category->name}}-এর সবগুলো</a>
											  	<ul class="modal-second-menu">
							...
							<div class="col-lg-12 col-sm-6 col-sm-12">
								<div class="ads-breadcrumb">
								  <ul>
									...
									<li><a class="anchor">@if($findthana !=NULL) {{$finddivision->name}} <i class="fa fa-angle-right"></i>  {{$finddistrict->dist_name}} <i class="fa fa-angle-right"></i> {{$findthana->thana_name}} @else অবস্থান নির্বাচন করুন @endif </a></li>
								</ul>
								<p>@if($findthana !=NULL) {{$finddivision->name}} <i class="fa fa-angle-right"></i>  {{$finddistrict->dist_name}} <i class="fa fa-angle-right"></i> {{$findthana->thana_name}} @else অবস্থান নির্বাচন করুন @endif  এর জন্য  {{$advertisments->count()}} টি  বিজ্ঞাপন খুঁজে পাওয়া গেছে </p>
								</div>
							</div>
							<!-- col end -->
						</div>
